...
def affine_forward(x, w, b):
    ...
    ###########################################################################
    x_vec = x.reshape(x.shape[0], -1) # x_NxD flattened/vectorized
    b = b.reshape(1, -1) # b_1xM
    out = (x_vec @ w) + b # out_NxM
    pass
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
...
def affine_backward(dout, cache):
    ...
    ###########################################################################
    x_vec = x.reshape(x.shape[0], -1) # x_NxD flattened/vectorized
    dw = x_vec.T @ dout # dw_DxM
    db = np.sum(dout, axis=0) # db_Mx1
    dx = dout @ w.T
...
def dropout_forward(x, dropout_param):
    ...
    if mode == 'train':
        ...
        #######################################################################
        mask = (np.random.binomial(1, p, size=x.shape) < p) / p
        out = x * mask
...
def spatial_batchnorm_forward(x, gamma, beta, bn_param):
    ...
    ###########################################################################
    N, C, H, W = x.shape
    x_reshaped = x.transpose(0,2,3,1).reshape(N*H*W, C) # (N, D)
    out_tmp, cache = batchnorm_forward(x=x_reshaped, gamma=gamma, beta=beta, bn_param=bn_param)
...
def spatial_batchnorm_backward(dout, cache):
    ...
    dx_reshaped, dgamma, dbeta = batchnorm_backward(cache=cache, dout=dout_tmp)
...
def conv_forward_naive(x, w, b, conv_param):
    ...
    # Make sure the resukting h_out and w_out are integers
    if not h_out.is_integer() or not w_out.is_integer():
        raise Exception('Invalid output dimension!')

    # Type cast the resulting h and w output size
    h_out, w_out = int(h_out), int(w_out)

    # Changing convolution to matrix multiplication with im2col
    X_col = im2col_indices(X=x, field_height=h_filter, field_width=w_filter, padding=padding, stride=stride)
    ...
    #     conv_param = stride, padding, X_col
...
def conv_backward_naive(dout, cache):
    ...
    dX_col = w.T @ dout
    dx = col2im_indices(X_col=dX_col, X_shape=x.shape, field_height=h_filter, field_width=w_filter, 
...
def max_pool_forward_naive(x, pool_param):
    ...
    X_col = im2col_indices(X=X_reshaped, field_height=pool_height, field_width=pool_width, 
...
def max_pool_backward_naive(dout, cache):
    ...
    dx = col2im_indices(X_col=dX_col, X_shape=(n * d, 1, h, w), field_height=pool_height, 
