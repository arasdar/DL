...
class SPNN(nn.NN):
    ...
    def forward(self, X, train=False):
        ...
        # midst layer: Convnet
        h2, h2_cache = l.conv_forward(h1, self.model['W2'], self.model['b2'])
    ...
    def backward(self, y_pred, y_train, cache):
        X, h1_cache, h2_cache, h3_cache, nl_cache1, nl_cache2, u2, bn2_cache = cache

        # Output layer
        dy = self.dloss_funs[self.loss](y_pred, y_train)

        # Third layer
        dh2, dW3, db3 = l.fc_backward(dy, h3_cache)
        ...
        dX_, dW1, db1 = l.conv_backward(dh1, h1_cache)
    ...
    def _init_model(self, D, C, H):
        ...
        self.bn_caches = dict(            
            bn2_mean=np.zeros((H, 1)),
            bn2_var=np.zeros((H, 1))
        )